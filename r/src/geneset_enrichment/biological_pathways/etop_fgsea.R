...
dir.create("results/output_encode/Etop/fgsea_enrichment")


data_file = "~/bs-villungerlab/results/output_encode_1to6/Etop_unfiltered_results_files.Rdata"
if (file.exists(data_file)){
  load(data_file)
  print("file_exists")
} else {
  results_Etop_t8 <- get_unfiltered_results(dds_Etop, "timepoint_t8_vs_t0", "_8")
  results_Etop_t12 <- get_unfiltered_results(dds_Etop, "timepoint_t12_vs_t0", "_12")
  results_Etop_t16 <- get_unfiltered_results(dds_Etop, "timepoint_t16_vs_t0", "_16")
  results_Etop_t24 <- get_unfiltered_results(dds_Etop, "timepoint_t24_vs_t0", "_24")
...
subset_df_2n <- abs_foldchange_increase[
  !is.na(abs_foldchange_increase$log2FoldChange_8) & !is.na(abs_foldchange_increase$log2FoldChange_12) |
    !is.na(abs_foldchange_increase$log2FoldChange_12) & !is.na(abs_foldchange_increase$log2FoldChange_16) |
    !is.na(abs_foldchange_increase$log2FoldChange_16) & !is.na(abs_foldchange_increase$log2FoldChange_24) |
    !is.na(abs_foldchange_increase$log2FoldChange_24) & !is.na(abs_foldchange_increase$log2FoldChange_48)
  , ]
...
write.csv(fgseaRes_increase_3n_print, file = "results/output_encode/Etop/fgsea_enrichment/fgsea_enrichment_Etop_increase_3n.csv")


## 2
sorted_subset_foldchange_increase_2n <- subset_df_2n[order(-subset_df_2n$log2FoldChange_16), ]

sorted_foldchange_increase_2n_tovec <- data.frame(gene_symbol = sorted_subset_foldchange_increase_2n$symbol, log2foldchange <- sorted_subset_foldchange_increase_2n$log2FoldChange_16)
...
write.csv(fgseaRes_increase_2n_print, file = "results/output_encode/Etop/fgsea_enrichment/fgsea_enrichment_Etop_increase_2n.csv")
...
write.csv(significant_pathways_3n_print, file = "results/output_encode/Etop/fgsea_enrichment/significant_pathways_3n.csv")
...
df_long_increase <- generate_complete_long_df(subset_df_upregulated, 16)
df_long_decrease <- generate_complete_long_df(subset_df_downregulated, 16)
...
saveWidget(interactive_increase_plot, "results/output_encode/Etop/fgsea_enrichment/Etop_interactive_increase_plot.html")
...
saveWidget(interactive_decrease_plot, "results/output_encode/Etop/fgsea_enrichment/Etop_interactive_decrease_plot.html")
