...
---
title: "Salmon Analysis - Noc_r1to6 "
...
load("../../data/Noc_r1to6_data.RData")
dds
results <- results(dds)
resultsNames(dds)
...
head(results_ordered_results_t0_t16)

results_ordered_results_t0_t16_df <- as.data.frame(results_ordered_results_t0_t16)[1:100, ]
write.csv(results_ordered_results_t0_t16_df, file = "results/Noc_r1to6_data.csv")
...
##### VST - Variance Stablised Transformation
```{r}
vsd_results_t20_t0 <- vst(dds, blind=FALSE)
plotPCA(vsd_results_t20_t0, intgroup=c('batch', 'timepoint'))
```
##### rLog
```{r}
rld_results_t20_t0 <- rlog(dds, blind=FALSE)
plotPCA(rld_results_t20_t0, intgroup=c('batch', 'timepoint'))
```
#### Heatmap
##### Euclidian Distance
...
```{r}
sampleDists_results_t20_t0 <- dist(t(assay(vsd_results_t20_t0)))

sampleDistMatrix_results_t20_t0 <- as.matrix( sampleDists_results_t20_t0 )
rownames(sampleDistMatrix_results_t20_t0) <- paste( vsd_results_t20_t0$timepoint, vsd_results_t20_t0$batch, sep = "_" )
colnames(sampleDistMatrix_results_t20_t0) <- NULL
colors <- colorRampPalette( rev(brewer.pal(9, "Blues")) )(255)
pheatmap(sampleDistMatrix_results_t20_t0,
         clustering_distance_rows = sampleDists_results_t20_t0,
         clustering_distance_cols = sampleDists_results_t20_t0,
         col = colors)
```
...
```{r}
poisd_results_t20_t0 <- PoissonDistance(t(counts(dds)))

samplePoisDistMatrix_results_t20_t0 <- as.matrix( poisd_results_t20_t0$dd )
rownames(samplePoisDistMatrix_results_t20_t0) <- paste( vsd_results_t20_t0$timepoint, vsd_results_t20_t0$batch, sep = "_" )
colnames(samplePoisDistMatrix_results_t20_t0) <- NULL
pheatmap(samplePoisDistMatrix_results_t20_t0,
         clustering_distance_rows = poisd_results_t20_t0$dd,
         clustering_distance_cols = poisd_results_t20_t0$dd,
         col = colors)
```
...
We can see that there is a clear batch effect.
```{r}
vsd_results_t0_t20 <- vst(dds, blind=FALSE)
plotPCA(vsd_results_t0_t20, intgroup=c('batch', 'timepoint'))

```

Using limma we can normalise the counts for this.
```{r}
mat_results_t0_t20 <- assay(vsd_results_t0_t20)
mm_results_t0_t20 <- model.matrix(~timepoint, colData(vsd_results_t0_t20))
mat_results_t0_t20 <- limma::removeBatchEffect(mat_results_t0_t20, batch=vsd_results_t0_t20$batch, design=mm_results_t0_t20)
assay(vsd_results_t0_t20) <- mat_results_t0_t20
plotPCA(vsd_results_t0_t20, intgroup=c('batch', 'timepoint'))
...
```{r}
design_results_t0_t20 <- model.matrix(~ timepoint, colData(vsd_results_t0_t20))
fit_results_t0_t20 <- lmFit(mat_results_t0_t20, design_results_t0_t20)
# Replace 'treatment' and 'control' with your specific time points.
...
##### VST - Variance Stablised Transformation
```{r}
vsd_results_t36_t0 <- vst(dds, blind=FALSE)
plotPCA(vsd_results_t36_t0, intgroup=c('batch', 'timepoint'))
```
##### rLog
```{r}
rld_results_t36_t0 <- rlog(dds, blind=FALSE)
plotPCA(rld_results_t36_t0, intgroup=c('batch', 'timepoint'))
```
#### Heatmap
##### Euclidian Distance
...
```{r}
sampleDists_results_t36_t0 <- dist(t(assay(vsd_results_t36_t0)))

sampleDistMatrix_results_t36_t0 <- as.matrix( sampleDists_results_t36_t0 )
rownames(sampleDistMatrix_results_t36_t0) <- paste( vsd_results_t36_t0$timepoint, vsd_results_t36_t0$batch, sep = "_" )
colnames(sampleDistMatrix_results_t36_t0) <- NULL
colors <- colorRampPalette( rev(brewer.pal(9, "Blues")) )(255)
pheatmap(sampleDistMatrix_results_t36_t0,
         clustering_distance_rows = sampleDists_results_t36_t0,
         clustering_distance_cols = sampleDists_results_t36_t0,
         col = colors)
```
...
```{r}
poisd_results_t36_t0 <- PoissonDistance(t(counts(dds)))

samplePoisDistMatrix_results_t36_t0 <- as.matrix( poisd_results_t36_t0$dd )
rownames(samplePoisDistMatrix_results_t36_t0) <- paste( vsd_results_t36_t0$timepoint, vsd_results_t36_t0$batch, sep = "_" )
colnames(samplePoisDistMatrix_results_t36_t0) <- NULL
pheatmap(samplePoisDistMatrix_results_t36_t0,
         clustering_distance_rows = poisd_results_t36_t0$dd,
         clustering_distance_cols = poisd_results_t36_t0$dd,
         col = colors)
```
...
We can see that there is a clear batch effect.
```{r}
vsd_results_t0_t36 <- vst(dds, blind=FALSE)
plotPCA(vsd_results_t0_t36, intgroup=c('batch', 'timepoint'))

```

Using limma we can normalise the counts for this.
```{r}
mat_results_t0_t36 <- assay(vsd_results_t0_t36)
mm_results_t0_t36 <- model.matrix(~timepoint, colData(vsd_results_t0_t36))
mat_results_t0_t36 <- limma::removeBatchEffect(mat_results_t0_t36, batch=vsd_results_t0_t36$batch, design=mm_results_t0_t36)
assay(vsd_results_t0_t36) <- mat_results_t0_t36
plotPCA(vsd_results_t0_t36, intgroup=c('batch', 'timepoint'))
...
```{r}
design_results_t0_t36 <- model.matrix(~ timepoint, colData(vsd_results_t0_t36))
fit_results_t0_t36 <- lmFit(mat_results_t0_t36, design_results_t0_t36)
# Replace 'treatment' and 'control' with your specific time points.
