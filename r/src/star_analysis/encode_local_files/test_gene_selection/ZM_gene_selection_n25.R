...
upr_top_merged_df <- merge_all_data(upr_top, results_ZM_t16_df, results_ZM_t20_df, results_ZM_t36_df, results_ZM_t48_df, 'results/output_encode/n25generegulation/ZM_gene_regulation_data.csv')
...
upr_plot
ggsave(filename = "results/output_encode/n25generegulation/ZM_upregulated_genes.pdf", plot = upr_plot)

downr_df_sorted <- df[order(df$log2FoldChange), ]
# set the number of results which you want
downr_top <- head(upr_df_sorted, 25)
downr_top_merged_df <- merge_all_data(downr_top, results_ZM_t16_df, results_ZM_t20_df, results_ZM_t36_df, results_ZM_t48_df, 'results/output_encode/n25generegulation/ZM_gene_regulation_data.csv')
...
downr_plot
ggsave(filename = "results/output_encode/n25generegulation/ZM_downregulated_genes.pdf", plot = downr_plot)

#####
df 
dim(df)
# This join will conditionally affect the results, because it will restrict it based if the values are already there.
all_df_merged_df <- merge_all_data(results_ZM_t48_df, results_ZM_t16_df, results_ZM_t20_df, results_ZM_t24_df, results_ZM_t36_df, 'results/output_encode/all_ZM_gene_regulation_data.csv', 'full_join')
...
# 
# # 'results/output_encode/all_ZM_gene_regulation_data.csv'
# write.csv(subset_df, file = "results/output_encode/ZM_all_timepoints.csv")
# subset_long_df <- make_longdf_for_plot(subset_df, 24)
# subset_plot <- plot_longdf(subset_long_df, "ZM genes (all timepoints)")
# subset_plot

# ggsave(filename = "results/output_encode/ZM_all_timepoints.pdf", plot = subset_plot)
...
write.csv(semi_subset_df, file = "results/output_encode/ZM_middle_timepoints.csv")
ggsave(filename = "results/output_encode/ZM_middle_timepoints.pdf", plot = semi_subset_plot)
######
...
ggsave(filename = "results/output_encode/three_consecutive_values.pdf", plot = three_p)
