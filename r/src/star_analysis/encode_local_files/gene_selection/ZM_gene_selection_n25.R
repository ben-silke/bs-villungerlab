...
ggsave(filename = "results/output_encode/ZM/ZM_middle_timepoints.pdf", plot = semi_subset_plot, dpi=dpi, width=width_in, height=height_in)
######
...
plot_title <- "Full Signature ZM treatment Signature"
...
ggsave(filename = "results/output_encode/ZM/ZM_full_signature_plot.pdf", plot = two_p, dpi=dpi, width=width_in, height=height_in)
...
three_consecutivesubset_df <- df[(!is.na(df$log2FoldChange_16) & !is.na(df$log2FoldChange_20) & !is.na(df$log2FoldChange_24) |
...
# set the number of results which you want
upr_top <- head(upr_df_sorted, 25)
upr_top_merged_df <- merge_all_data(upr_top, results_ZM_t16_df, results_ZM_t20_df, results_ZM_t36_df, results_ZM_t48_df, 'results/output_encode/ZM/n25generegulation/ZM_full_5n_gene_regulation_data.csv')

df <- upr_top_merged_df
